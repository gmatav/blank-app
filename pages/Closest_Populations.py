...
# Sidebar (controls)
# -----------------------------
st.sidebar.title("⚙️ Closest Populations Settings")
...
# Info display
st.info(f"Top {top_n} results using: {', '.join(metrics)}")
...
if st.button("Compute distances"):
    ...
    for m in metrics:
        ...
        # ✅ Fix wide columns (truncate long names)
        df_res["Population"] = df_res["Population"].str.slice(0, 40)
